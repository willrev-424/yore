...
local function ColorInt(c)
	return type(c) == "number" and c or 0xFF99CC
end
...
function WebhookConfig.new(name, cfg)
	...
	self.URL = cfg.URL or ""
	self.Username = cfg.Username or "MugiHub"
	...
	self.Color = cfg.Color or 0xFF99CC
	self._queue = {}
	self._events = {}
	self._listeners = {}
...
local MugiWebhook = {}
MugiWebhook._configs = {}

function MugiWebhook:NewConfig(name, cfg)
	local c = WebhookConfig.new(name, cfg)
	self._configs[name] = c
	return c
end

function MugiWebhook:Get(name)
	return self._configs[name]
end

function MugiWebhook:AttachToLibrary(lib, configNameOrObj)
...
-- ============================================
-- CUSTOM THEME (MODIFIED: Hitam Monochrome)
-- ============================================
local Custom = {}
do
	-- UBAH: Warna aksen jadi abu netral (hapus pink)
	Custom.ColorRGB = Color3.fromRGB(200, 200, 200)
	-- UBAH: Gradient abu ke putih (hapus pink)
	Custom.GradientPinkWhite = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(180, 180, 180)),
		ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 255, 255)),
	})
	-- UBAH: Default icon dikosongkan (hapus logo Mugi)
	Custom.DefaultIcon = ""
...
-- UBAH: Tombol floating tanpa logo Mugi (pakai teks "M")
local function OpenClose()
	local Gui = Custom:Create("ScreenGui", { ZIndexBehavior = Enum.ZIndexBehavior.Sibling }, Custom:GetGuiParent())
	...
	Custom:Create("UICorner", { CornerRadius = UDim.new(1, 0) }, Btn)
	Custom:Create("UIStroke", { Color = Color3.fromRGB(60, 60, 60), Thickness = 1.5 }, Btn)
...
function MugiHub_Library:CreateWindow(Config)
	...
	-- UBAH: TabWidth dari 105 -> 115
	local TabWidth = Config[3] or Config["Tab Width"] or 115
	-- UBAH: SizeUi dari 480x275 -> 530x305
	local SizeUi = Config[4] or Config.SizeUi or UDim2.fromOffset(530, 305)
	local Keybind = Config[5] or Config.Keybind or Enum.KeyCode.RightControl
	-- UBAH: Default icon jadi kosong (hapus logo Mugi)
	local Icon = Config[6] or Config.Icon or ""
	local Funcs = {}
	local SearchRegistry = {}

	local MugiHubGui = Custom:Create("ScreenGui", { ZIndexBehavior = Enum.ZIndexBehavior.Sibling }, Custom:GetGuiParent())

	-- UBAH: DropShadowHolder dari 400x310 -> 440x340
	local DropShadowHolder = Custom:Create("Frame", {
		BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(0, 440, 0, 340), ZIndex = 0,
		Name = "DropShadowHolder",
		Position = UDim2.new(0, (MugiHubGui.AbsoluteSize.X // 2 - 440 // 2), 0, (MugiHubGui.AbsoluteSize.Y // 2 - 340 // 2)),
	}, MugiHubGui)
	...
	ExitButton.Activated:Connect(function()
		...
		task.spawn(function() HideExitConfirm() if MugiHubGui then MugiHubGui:Destroy() end end)
	end)
	...
	function Funcs:AddTab(Config)
		...
		local function SelectTab()
			...
			ActiveTab = TabFuncs TabFrame.Visible = true NameTab.Text = TabName
		...
		function TabFuncs:AddSection(SectionConfig)
			...
			function SecFuncs:AddCopyGroup(CopyConfig)
				...
				CopyBtn.Activated:Connect(function()
					...
					if setclipboard then setclipboard(CopyText) MugiHub_Library:SetNotification({ Content = "Copied: " .. CopyText }) end
				end)
